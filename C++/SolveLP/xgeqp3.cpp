...
void xgeqp3(double A[21904], int m, int n, int jpvt[148], double tau[148])
{
  ...
  std::memset(&tau[0], 0, 148U * sizeof(double));
  if (minmn_tmp < 1) {
    ...
    if (nfxd < minmn_tmp) {
      ...
      std::memset(&vn1[0], 0, 148U * sizeof(double));
      std::memset(&vn2[0], 0, 148U * sizeof(double));
      i = nfxd + 1;
      for (pvt = i; pvt <= n; pvt++) {
        d = blas::xnrm2(m - nfxd, A, (nfxd + (pvt - 1) * 148) + 1);
        vn1[pvt - 1] = d;
        vn2[pvt - 1] = d;
      }
      for (int b_i{i}; b_i <= minmn_tmp; b_i++) {
        ...
        if (b_i < n) {
          temp = A[ii];
          A[ii] = 1.0;
          reflapack::xzlarf(mmi + 1, nmi - 1, ii + 1, d, A, ii + 149, work);
          A[ii] = temp;
        }
