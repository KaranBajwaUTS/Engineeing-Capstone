...
void modifyOverheadPhaseOne_(c_struct_T *obj)
{
  ...
  i = obj->sizes[0];
  for (idx = 0; idx < i; idx++) {
    obj->ATwset[(obj->nVar + 75 * idx) - 1] = 0.0;
  }
  obj->Aeq[obj->nVar - 1] = 0.0;
  obj->ATwset[(obj->nVar + 75 * (obj->isActiveIdx[1] - 1)) - 1] = 0.0;
  for (idx = 0; idx < 48; idx++) {
    obj->Aineq[(obj->nVar + 75 * idx) - 1] = -1.0;
  }
  ...
  for (idx = idxStartIneq; idx <= i; idx++) {
    obj->ATwset[(obj->nVar + 75 * (idx - 1)) - 1] = -1.0;
  }
